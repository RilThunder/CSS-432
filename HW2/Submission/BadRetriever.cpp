...
string FILENAME = "content.txt"; // Name of the file that will contain the information from the server
                                 // Will overwrite this file if the file exist
...
int processGetRequest(int socketFileDescriptor)
{
    ...
    cout << "Writing the content to " + FILENAME << endl;
    ofstream outputFile;
    outputFile.open(FILENAME);
    char buffer[length]; // Allocate a buffer to store the content
    ...
    outputFile.close();
    cout << "Finish writing the content of the response to the file " << endl;
    return 0;
...
int main(int argumentNumber , char *argumentValues[])
{
    if ( argumentNumber != 4 )
    {
        cout << "Incorrect number of argument provided. Expected to receive server address, file and port number ";
    }
